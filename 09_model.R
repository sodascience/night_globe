...
gan_grid <- read_rds("data/gan_grid_geoenrich.rds")
...
fit_model3 <- lm(frm, gan_compl)
prd3 <- predict(fit_model3, newdata = grid_compl, se.fit = TRUE)
...
frm <- as.formula(paste(
  ...
  paste(cn[str_detect(cn, "landtype")], collapse = "+"),
  paste("+ CloudCover + motorway_10km")
))

fit_model6 <- lm(frm, gan_compl)
prd6 <- predict(fit_model6, newdata = grid_compl, se.fit = TRUE)
pred_model6 <- grid_compl %>% mutate(est = prd6$fit, var = prd6$se.fit^2)
...
ggsave("img/landuse and OSM no kriging.png")
...
ggsave("img/landuse_OSM_kriging.png")
...
# comparing R²
tibble(
  Model = c("Naive", "Krige", "Landuse", "Landuse_Krige","OSM_Kriging","Landuse_OSM","Landuse_OSM_Krige"), 
